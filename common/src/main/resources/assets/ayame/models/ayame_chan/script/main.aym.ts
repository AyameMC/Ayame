...
PlayerEvents.tick((event) => {
    ...
    blinker.tick();
    Molang.setVariable('cus.is_blinking', blinker.isBlinking() ? 1 : 0);
})
...
RouletteOption.add("抱头蹲防", "minecraft:textures/item/diamond.png", (event) => {
    const { player } = event
    // player.playSound('ayame:models/ayame_chan/zufolo_impazzito.ogg', 1.0, 1.0);
    player.spawnParticle("minecraft:angry_villager", player.getX() + 0.5, player.getY() + 0.5, player.getZ() + 0.5, 0.0, 0.0, 0.0)
