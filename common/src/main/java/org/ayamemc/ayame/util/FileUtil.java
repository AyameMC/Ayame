...
public class FileUtil {
    ...
    public static void copyBuiltinDirectoryToDirectory(String sourcePath, String targetPath) {
        final Path targetPathDir = Path.of(targetPath);
        try {
            final File currentJar = new File(FileUtil.class.getProtectionDomain().getCodeSource().getLocation().toURI());

            try (JarFile jarFile = new JarFile(currentJar)) {
                ...
                while (entries.hasMoreElements()) {
                    ...
                    if (name.startsWith(sourcePath)) {
                        final File target = targetPathDir.resolve(name).toFile();
